...
from torch.utils.tensorboard import SummaryWriter  # For logging data to TensorBoard
import pandas as pd  # For data manipulation and analysis
from stable_baselines3.common.callbacks import EvalCallback  # Evaluation callback for periodic performance testing
...
meta_model_path = os.path.join(SAVED_MODEL_PATH, 'Adapted_Model')  # Path for saving/loading a meta-trained model
...
class CustomEvalCallback(EvalCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls % self.eval_freq == 0:  # Check if it's time for evaluation
            self.rewards.append(self.last_mean_reward)  # Store the last evaluation's mean reward
...
def build_model(env):
    model = PPO("MlpPolicy", env, verbose=0, learning_rate=0.0001, batch_size=20, n_steps=SIM_TIME)  # Initialize PPO with given settings
    return model

# Function to load an existing meta-trained model for adaptation to new environments
def load_model(env):
    meta_model = PPO("MlpPolicy", env, verbose=0, learning_rate=0.0001, batch_size=20, n_steps=SIM_TIME)  # Initialize a new model structure
    meta_saved_model = PPO.load(meta_model_path)  # Load pretrained model weights from path
    meta_model.policy.load_state_dict(meta_saved_model.policy.state_dict())  # Transfer weights to the new model structure
    return meta_model
...
def make_scenario():
    ...
    for mean in range(2):  # Range of mean lead time values
        ...
        for std in range(1):  # Range of standard deviations
            ...
            leadtime_scenario.append(leadtime_dict)  # Append each lead time scenario
...
for demand_scenario_dict in demand_scenario:
    for leadtime_scenario_dict in leadtime_scenario:
        ...
        rl_log_path = os.path.join(EXPERIMENT_LOGS, f'RANDOM_case_{case_num}')  # Path for RL experiment logs
        os.makedirs(rl_log_path)  # Create directory for logs
        rl_env.writer = SummaryWriter(rl_log_path)  # Initialize TensorBoard writer
        ...
        meta_log_path = os.path.join(EXPERIMENT_LOGS, f'META_case_{case_num}')  # Path for Meta experiment logs
        os.makedirs(meta_log_path)  # Create directory for logs
        meta_env.writer = SummaryWriter(meta_log_path)  # Initialize TensorBoard writer
